...
class ShellUploader():
    ...
    def showSettingsUI(self, parentWidget):
        ...
        self.settingsDialog.group_shell.output_is_url.checked = \
            self.outputIsUrl
    ...
    def upload(self, screenshot, name):
        ...
        command = string.Formatter().vformat(
                self.commandFormat, (), defaultdict(str, s=tmpFilename))
        try:
            command = command.encode(sys.getfilesystemencoding())
        except UnicodeEncodeError:
            ScreenCloud.setError(
                    "Invalid characters in command '" + command + "'")
            return False
        try:
            if self.outputIsUrl:
            ...
            if p.returncode > 0:
                ScreenCloud.setError(
                        "Command " + command + " did not return 0")
                return False
